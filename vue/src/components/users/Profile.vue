<template>
  <div>
    ...
    <div v-else class="flex justify-center bg-gray-900">
      <div class="h-full p-8 max-w-7xl relative">
          <div class=" rounded-lg shadow-xl pb-8">
              <Disclosure v-if="!(route.params.slug || route.query.id)" as="div" class="absolute right-12 mt-4 rounded" >
                      <Menu as="div" class="relative ml-3">
                        <MenuButton  class="border border-gray-400 p-2 rounded text-gray-300 hover:text-gray-300 bg-transparent bg-opacity-10 hover:bg-opacity-20">
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"></path>
                            </svg>
                        </MenuButton>
                        <transition enter-active-class="transition ease-out duration-100 " enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                          <MenuItems class="bg-white absolute right-0 w-40 mt-1 shadow-2xl py-2 border-b rounded-md">
              ...
              <div class="flex flex-col items-center -mt-20">
                <div class="w-40 h-40 border-4 border-white rounded-full flex items-center justify-center overflow-hidden shadow-lg bg-white">
                  <img :src="user.image_url"  class="object-cover w-40 h-40 rounded-full ring-4 ring-gray-300">
                </div>
                  <div class="flex items-center space-x-2 mt-2">
                      <p class="text-2xl font-bold font-serif text-gray-300">{{ user.name }}</p>
                  ...
                  <p class="text-gray-500">{{ user.role == 'Student' ? user.role: user.position  }}</p>
                  <p class="text-sm text-gray-500">{{ user.department }}</p>
              </div>
              <div v-if="route.params.slug" class="flex items-center lg:items-end justify-end px-8 mt-2">
                  <div class="flex items-center space-x-4 mt-2">
                      <a :href="`/make-appointment/${user.id}`" class="flex items-center bg-blue-600 hover:bg-blue-700 text-gray-100 px-4 py-2 rounded text-sm space-x-2 transition duration-100">
              ...
              <div class="w-full flex flex-col 2xl:w-1/3">
                  <div class="flex-1 bg-white rounded-lg shadow-xl p-8">
                      ...
                      <ul class="mt-2 text-gray-700">
                          ...
                          <li class="flex border-y py-2">
                              <span class="font-bold w-24">Courses</span>
                              <span class="text-gray-700">{{ user.course }}</span>
                          </li>
                          <li class="flex border-b py-2">
                              <span class="font-bold w-24">Birthday:</span>
                              <span class="text-gray-700">{{ user.birth_date }}</span>
                          </li>
                          <li class="flex border-b py-2">
                              <span class="font-bold w-24">Joined:</span>
                              <span class="text-gray-700">{{ user.joining_date }}</span>
                          </li>
                          <li class="flex border-b py-2">
                              <span class="font-bold w-24">Mobile:</span>
                              <span class="text-gray-700">{{ user.contact_number }}</span>
                          </li>
                          <li class="flex border-b py-2">
                          ...
                          <li class="flex border-b py-2">
                              ...
                              <span class="text-gray-700">{{ user.languages }}</span>
                          </li>
...
const userNavigation = [
  { name: 'Edit your Profile', href: '#', function: ()=>{
    router.push({
      name: 'EditProfile',
    })
  }},
  { name: 'Make schedule', href: '#', function: ()=>{
    router.push({
      name: 'MakeScheduleForAppointment'
    })
  }},
...
onMounted(()=>{
  ...
  if(route.params.slug || route.query.id){
    store.dispatch('facultyMember', route.params.slug || route.query.id)
      .then(()=>{
