                      ...
                      <div class="overflow-x-auto">
                        ...
                        <div v-else v-for="(appointment, ind) in appointments" :key="ind"
                          ...
                          <div class="w-full md:w-1/3 bg-white grid place-items-center">
                            <img :src="appointment.host.image_url" alt="tailwind logo"  class="rounded-xl" />
                          </div>
                            <div class="w-full md:w-2/3 bg-white flex flex-col space-y-2 p-3">
                              <div class="flex justify-between item-center">
                                <p class="text-gray-500 font-medium hidden md:block">{{ appointment.host.role }}</p>
                                <p class="text-gray-500 font-medium hidden md:block">{{ appointment.host.department }}</p>

                              </div>
                              <a :href="`/profile/${appointment.host.id }`" class="font-black text-blue-700 underline md:text-3xl text-xl">{{ appointment.host.name }}</a>
                              <div>
...
const props = defineProps(['title', 'appointments', 'loading', 'message', 'paginationLinks']);
